...
class Scribendi_Api_Client_Adaptor_Order extends Scribendi_Api_Client_Adaptor_Abstract {
    ...
    const VALIDATE_RETURNED = 'returned';
	const ERROR_CODE_ORDER_INVALID = 1010;
	const ERROR_CODE_ORDER_CANCELLED = 1020;
	protected function initialise() {
		$this->setName('order');
    ...
    function setOrderReturned($inOrderId = null) {
        $this->getApiQuery()->setOptions(array(Scribendi_Api_Constants::FIELD_ORDER_ID => $inOrderId));
        $this->setValidationType(self::VALIDATE_RETURNED);
        $this->_setRequestUri('returned');
        $this->addRequiredParameters(Scribendi_Api_Constants::FIELD_ORDER_ID);
        $this->getApiQuery()->setOptions(array(Scribendi_Api_Constants::FIELD_REQUEST_SIGNATURE => $this->getRequestSignature('POST')));
        $this->getTransportOptions()->setOptions(
            array(
                CURLOPT_POST => true,
	...
	function addFileToOrder($inOrderID = null, $inFile = null) {
        ...
        if ( $inOrderID !== null && $inFile !== null ) {
            ...
            if ( ! is_resource($inFile) ) {
                ...
                // Add @ to file path
                if ( strpos($inFile, '@') !== 0 ) {
                    $postFile = $inFile = '@' . $inFile;
                }

                // Use CURLFile if class exists
                if ( class_exists('CURLFile') ) {
                    $fInfo      = finfo_open(FILEINFO_MIME_TYPE);
                    $postFile   = new CURLFile(substr($inFile, 1), finfo_file($fInfo, substr($inFile, 1)), basename($postFile));
                    finfo_close($fInfo);
                }
            }
            ...
            $this->getApiQuery()->setOptions(
                array(
                    ...
                    Scribendi_Api_Constants::FIELD_ORDER_FILE_UPLOAD => $postFile,
                )
            );
        }
		...
		if ( ! is_resource($inFile)) {
			$filePath = substr($inFile, 1);
			if ( ! file_exists($filePath) || ! is_readable($filePath) ) {
				throw new Scribendi_Api_Client_Exception('File '.$filePath.' cannot be read or does not exist');
			}
		}
		$this->getTransportOptions()->setOptions(
			array(
